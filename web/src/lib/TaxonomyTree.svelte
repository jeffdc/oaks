<script>
  import { taxonomyTree, sortTaxonomyKeys, formatSpeciesName } from './dataStore.js';
...
<div class="taxonomy-tree">
  <div class="tree-header">
    <span class="header-label">Browse by Taxonomy</span>
  </div>

  <div class="tree-container">
    {#each sortTaxonomyKeys(Object.keys($taxonomyTree)) as subgenus}
      {@const subgenusData = $taxonomyTree[subgenus]}
      {@const subgenusKey = subgenus}
      {@const hasSubgenus = subgenus && subgenus !== 'null'}
      ...
      {#if hasSubgenus}
        ...
        <div class="tree-node subgenus-node">
          <button
            class="node-header"
            on:click={() => toggleExpand(subgenusKey)}
          >
              ...
              {#each sortTaxonomyKeys(Object.keys(subgenusData.sections)) as section}
                ...
                {#if hasSection}
                  ...
                  <div class="tree-node section-node">
                    <button
                      class="node-header"
                      on:click={() => toggleExpand(sectionKey)}
                    >
                        ...
                        {#each sortTaxonomyKeys(Object.keys(sectionData.subsections)) as subsection}
                          ...
                          {#if hasSubsection}
                            ...
                            <div class="tree-node subsection-node">
                              ...
                              >
                                <span class="expand-icon">{expanded[subsectionKey] ? '▼' : '▶'}</span>
                                  ...
                                  {#each sortTaxonomyKeys(Object.keys(subsectionData.complexes)) as complex}
                                    ...
                                    {#if hasComplex}
                                      ...
                                      <div class="tree-node complex-node">
                                        <button
                                          class="node-header"
                                          on:click={() => toggleExpand(complexKey)}
                                        >
                                          <span class="expand-icon">{expanded[complexKey] ? '▼' : '▶'}</span>
                                          <span class="node-label">Complex Q. {complex}</span>
                                          <span class="node-count">({complexData.species.length})</span>
                                        </button>
      ...
      {:else}
        <!-- Species without subgenus - show at root level -->
        {#each sortTaxonomyKeys(Object.keys(subgenusData.sections)) as section}
          {@const sectionData = subgenusData.sections[section]}
          {#each sortTaxonomyKeys(Object.keys(sectionData.subsections)) as subsection}
            {@const subsectionData = sectionData.subsections[subsection]}
