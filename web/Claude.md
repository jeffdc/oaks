...
- **Framework**: Svelte 5 (latest features including runes)
- **Build Tool**: Vite 7
...
web/
├── src/
│   ├── App.svelte           # Root component with routing & history management
│   ├── main.js              # Entry point
│   ├── app.css              # Global styles & CSS custom properties
│   └── lib/
│       ├── dataStore.js     # Svelte stores for state management
│       ├── Search.svelte    # Search input component
│       ├── SpeciesList.svelte    # Species list view with counts
...
- **Client-side routing** using browser History API (no router library)
- State stored in `history.state` for back/forward navigation
- URL hash reflects current species: `#Quercus alba`
...
- Root component
- Manages view state (`list` or `detail`)
- Handles browser history navigation
- Header changes based on view (back button vs title + search)
...
- **Parent → Child**: Props
- **Child → Parent**: Function props (callbacks)
- **Global State**: Svelte stores (imported from `dataStore.js`)
...
2. Import stores from `dataStore.js` as needed
