...
The iOS app is being developed to provide a native field note experience. This change defines how the iOS app syncs with the central database via the API server (epic `oaks-w973`).
...
- Builds on existing `oaks-w973` epic work
...
**Endpoints** (building on `oaks-t41l` spec):
...
**What**: Simple API key authentication (initially)
...
- API key is sufficient for personal use
...
| leaf | leaf | leaves |
| acorn | acorn | fruits |
| bark | bark | bark_twigs_buds (bark portion) |
| buds | buds | bark_twigs_buds (buds portion) |
| form | form | growth_habit |
| rangeHabitat | rangeHabitat | range |
| fieldNotes | fieldNotes | miscellaneous |
| resources | resources | miscellaneous (appended) |

**Note**: API uses iOS field names for consistency. Server maps to DB columns internally.
...
1. **Phase 1**: Implement API server with notes endpoints (`oaks-doc9`)
2. **Phase 2**: Update iOS app to use API (`oaks-0ps1`)
...
1. Where to host API server? (Local Mac, Fly.io, Railway, home server?)
2. Should we add sync status indicator in iOS app?
3. Do we want conflict detection (vs pure last-write-wins)?
